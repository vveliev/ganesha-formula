...
We will see a quick setup to configure the ``ganesha`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``ganesha`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``ganesha`` formulas under ``/srv/salt/ganesha/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/ganesha/parameters/dns:domain/
    ln -s dns:domain /srv/salt/ganesha/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/ganesha/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/ganesha/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``ganesha`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``ganesha`` formulas under ``/srv/salt/ganesha/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/ganesha/parameters/roles
...
We create a configuration for the role ``ganesha/server`` in ``/srv/salt/ganesha/parameters/roles/ganesha/server.yaml``:
...
We create another configuration for the role ``ganesha/client`` in ``/srv/salt/ganesha/parameters/roles/ganesha/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles ganesha/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles ganesha/client
...
.. code-block:: console
    ...
    mv /srv/salt/ganesha/parameters/dns:domain/  '/srv/salt/ganesha/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-ganesha-as-json:
      test.nop:
        - name: {{ ganesha | json }}
...
Here is an example based on `template-formula/ganesha/config/file.sls`_:
...
This ``sls`` file expose a ``ganesha`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ ganesha['winner'] }}
...
.. _template-formula/ganesha/config/file.sls: https://github.com/saltstack-formulas/template-formula/blob/master/ganesha/config/file.sls
